...
<?php else: ?>
    ...
    <script>
        ...
        const DEFAULT_SCOPE_NAMES = {
            0: 'Local',
            1: 'Mesh',
        };

        function decodeScopeName(number) {
            const scopeNumber = Number.parseInt(number, 10);
            if (!Number.isFinite(scopeNumber) || scopeNumber < 0 || scopeNumber > 255) {
                return '';
            }

            if (Object.prototype.hasOwnProperty.call(DEFAULT_SCOPE_NAMES, scopeNumber)) {
                return DEFAULT_SCOPE_NAMES[scopeNumber];
            }

            return 'Scope ' + scopeNumber;
        }

        function normalizeScopeNameForKey(scopeName) {
            const trimmed = String(scopeName ?? '').trim();
            if (!trimmed) return '';
        ...
        async function deriveScopeNumberFromName(scopeName) {
            const normalized = normalizeScopeNameForKey(scopeName);
            ...
            return Number.parseInt(bytes[0], 10);
        }
        ...
        function addScopeRow(scope) {
            ...
            number.placeholder = scope.isAddRow ? 'Auto from Name' : '0-255';
            number.oninput = () => { number.style.color = '#1976D2'; };

            if (scope.isAddRow) {
                number.readOnly = true;
                number.title = 'Automatically generated from scope name using MeshCore key derivation';
            }
            ...
            let scopeHashRequestId = 0;
            const refreshScopeNumberFromName = () => {
                if (!scope.isAddRow) return;

                const requestId = ++scopeHashRequestId;
                deriveScopeNumberFromName(name.value)
                    .then((generated) => {
                        if (requestId !== scopeHashRequestId) return;

                        if (Number.isFinite(generated) && generated >= 0 && generated <= 255) {
                            number.value = String(generated);
                            number.dataset.autoGenerated = '1';
                            number.style.color = '#1976D2';
                        } else {
                            number.value = '';
                            number.dataset.autoGenerated = '0';
                        }
                    })
                    .catch(() => {
                        if (requestId !== scopeHashRequestId) return;
            ...
            name.oninput = () => {
                ...
                const decoded = decodeScopeName(number.value);
                name.dataset.autoGenerated = (decoded && name.value.trim() === decoded) ? '1' : '0';
                refreshScopeNumberFromName();
            };
            nameCell.append(name);

            const maybeAutofillScopeName = (markChanged) => {
                const decoded = decodeScopeName(number.value);
                if (!decoded) return;

                const current = name.value.trim();
                const autoGenerated = name.dataset.autoGenerated === '1';
                if (current === '' || autoGenerated) {
                    name.value = decoded;
                    name.dataset.autoGenerated = '1';
                    if (markChanged) {
                        name.style.color = '#1976D2';
                    }
                }
            ...
            const decodedInitial = decodeScopeName(number.value);
            if (!name.value.trim() && decodedInitial) {
                name.value = decodedInitial;
                name.dataset.autoGenerated = '1';
            } else {
                name.dataset.autoGenerated = (decodedInitial && name.value.trim() === decodedInitial) ? '1' : '0';
            }

            number.addEventListener('input', () => {
                number.style.color = '#1976D2';
                maybeAutofillScopeName(true);
            });
            ...
            if (scope.isAddRow) {
                ...
                saveBtn.onclick = () => {
                    ...
                    if (!data.name) {
                        data.name = decodeScopeName(data.number);
                    }
                    const formData = new FormData();
