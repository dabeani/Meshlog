...
- **Saved route points no longer lose valid collector paths just because the relay contact is old or the report was direct** — Historical route-trail popups now resolve stored reports against expired contacts when needed and fall back to the reporting collector when the stored repeater path is empty, instead of incorrectly showing “No repeater path stored” for valid saved reports.
...
- **Reporter time-sync lookup optimized** — Reporter time-sync enrichment now resolves the latest self-advertisement report with a grouped batched query instead of a correlated per-reporter lookup, reducing the remaining latency on `/admin/api/reporters/`.
...
- **Redundant snapshot refreshes reduced after initial page load** — After a successful `/api/v1/all/` load, the WebUI now seeds its live-map freshness timers and debounces scope refreshes so it does not immediately refetch large `contacts`, `reporters`, and `scopes` snapshots that were just delivered in the initial page payload.
- **Advertisement trail requests no longer fan out during popup refresh** — Device popup trail data now loads lazily from the active popup flow instead of being triggered as a side effect of general popup HTML generation, preventing bursts of per-contact `/api/v1/contact_advertisements/` requests.
...
- **MQTT worker restart hardening** — Single malformed packets or transient insert exceptions no longer tear down the whole MQTT loop; the worker now logs per-message failures and continues processing subsequent packets.
...
- **Incremental live-feed polling no longer skips same-second packets** — Shared `after_ms` query filters now use an inclusive timestamp boundary so second-resolution `created_at` values do not hide MQTT packets that arrive later within the same second as the current live-feed cursor.
...
- **Repeater neighbor overlays render again with collector filters active** — The map neighbor view for repeaters now treats synthetic neighbor/route-trail overlays as non-collector-owned paths, so `Show Neighbors` still works even when the WebUI collector filter is not set to all reporters.
- **Transient packet-flow animations now foreground only involved pins** — While a live packet-flow animation is visible, only the contacts on that animated route are lifted into the foreground; unrelated `marker-pin` contacts stay in the background instead of cutting across the flow.
- **Collector filter now suppresses hidden-reporter route visuals consistently** — When a collector is deselected in the WebUI settings, cached route overlays, hover/click path rendering, and live packet-flow animations now all ignore that reporter instead of drawing lines toward a hidden collector pin.
- **Packet-flow overlays no longer hide involved map pins** — Live packet route animations now render above uninvolved/background markers but below the active markers that belong to the shown route, so unrelated pins no longer cut across the flow while route endpoints stay visible.
