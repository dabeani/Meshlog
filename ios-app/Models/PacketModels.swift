...
struct StatisticsResponse: Decodable {
    ...
    let channelTotals: [ChannelStatsTotal]
    let note: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case channelTotals = "channel_totals"
        case note
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        channelTotals = (try? container.decode([ChannelStatsTotal].self, forKey: .channelTotals)) ?? []
...
struct CollectorStatsTotal: Decodable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case ctrlPackets = "ctrl_packets"
        case rawPackets = "raw_packets"
    }
}

struct ChannelStatsTotal: Decodable, Identifiable {
    let channelId: Int
...
struct Reporter: Identifiable, Codable {
    ...
    let authorized: Bool
    let lastHeardAt: String?
    let timeSync: ReporterTimeSync?
    let reporterStatus: ReporterStatus?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case reporterStatus = "reporter_status"
    }

    init(id: Int, publicKey: String, authorized: Bool, lastHeardAt: String?, timeSync: ReporterTimeSync?, reporterStatus: ReporterStatus?) {
        self.id = id
    ...
    init(from decoder: Decoder) throws {
        ...
        timeSync = try? container.decodeIfPresent(ReporterTimeSync.self, forKey: .timeSync)
        reporterStatus = try? container.decodeIfPresent(ReporterStatus.self, forKey: .reporterStatus)
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encodeIfPresent(timeSync, forKey: .timeSync)
        try container.encodeIfPresent(reporterStatus, forKey: .reporterStatus)
    }
}

struct ReporterStatus: Codable {
    let status: String?
    ...
    init(from decoder: Decoder) throws {
        ...
        radio = try? container.decodeIfPresent(String.self, forKey: .radio)

        if let intValue = try? container.decodeIfPresent(Int.self, forKey: .rssi) {
            rssi = intValue
        } else if let doubleValue = try? container.decodeIfPresent(Double.self, forKey: .rssi) {
            rssi = Int(doubleValue)
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .rssi) {
            rssi = Int(stringValue)
        } else {
            rssi = nil
        }

        if let doubleValue = try? container.decodeIfPresent(Double.self, forKey: .snr) {
            snr = doubleValue
        } else if let intValue = try? container.decodeIfPresent(Int.self, forKey: .snr) {
            snr = Double(intValue)
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .snr) {
            snr = Double(stringValue)
        } else {
            snr = nil
        }

        if let intValue = try? container.decodeIfPresent(Int.self, forKey: .batteryMv) {
            batteryMv = intValue
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .batteryMv) {
            batteryMv = Int(stringValue)
        } else {
            batteryMv = nil
        }

        if let intValue = try? container.decodeIfPresent(Int.self, forKey: .uptimeSecs) {
            uptimeSecs = intValue
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .uptimeSecs) {
            uptimeSecs = Int(stringValue)
        } else {
            uptimeSecs = nil
        }

        if let intValue = try? container.decodeIfPresent(Int.self, forKey: .queueLen) {
            queueLen = intValue
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .queueLen) {
            queueLen = Int(stringValue)
        } else {
            queueLen = nil
        }

        if let intValue = try? container.decodeIfPresent(Int.self, forKey: .errors) {
            errors = intValue
        } else if let stringValue = try? container.decodeIfPresent(String.self, forKey: .errors) {
            errors = Int(stringValue)
        } else {
            errors = nil
        }
    }
...
struct ReporterTimeSync: Codable {
    let available: Bool
    let warning: Bool
    let inSync: Bool?
    let driftMs: Double?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case latestContactType = "latest_contact_type"
    }
}
